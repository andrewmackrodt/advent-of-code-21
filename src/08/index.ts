...
export function countOccurrences(input: string, digits: number[]): number {
    return input.split('\n').reduce((total, entry) => (
        total + entry.split(' | ')[1]
            .split(' ')
            .filter(o => digits.includes(o.length)).length
    ), 0)
}

export function sumOutputs(input: string): number {
    let result = 0
    ...
    for (const line of input.split('\n')) {
        ...
        const assertOne = (predicate: (s: string) => boolean) => {
        ...
        const filterPattern = (length: number, pattern: string, common: number) => {
            return assertOne(s => (
                s.length === length && s.split('').filter(c => pattern.includes(c)).length === common
            ))
        }
