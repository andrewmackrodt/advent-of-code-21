"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.partTwo = exports.partOne = exports.getUniqueValidVelocityCount = exports.getMaxY = void 0;
const fs_1 = require("fs");
var SearchState;
(function (SearchState) {
    SearchState[SearchState["InProgress"] = 0] = "InProgress";
    SearchState[SearchState["Hit"] = 1] = "Hit";
    SearchState[SearchState["Miss"] = 2] = "Miss";
})(SearchState || (SearchState = {}));
function parseInput(input) {
    return input.replace(/ /g, '')
        .replace(/[^:]+:/, '')
        .split(',')
        .reduce((area, str) => {
        const [k, v1, v2] = str.split(/[=.]+/);
        area[`${k}1`] = parseInt(v1, 10);
        area[`${k}2`] = parseInt(v2, 10);
        return area;
    }, {});
}
function getVelocities(input) {
    const targetArea = parseInput(input);
    const searchRange = {
        x1: 0,
        x2: targetArea.x2,
        y1: targetArea.y1,
        y2: (targetArea.y1 * -1) - 1,
    };
    const trajectory = {
        x: 0,
        y: 0,
        xSpeed: 0,
        ySpeed: 0,
    };
    const reset = (initialSpeed) => {
        trajectory.x = 0;
        trajectory.y = 0;
        trajectory.xSpeed = initialSpeed.xSpeed;
        trajectory.ySpeed = initialSpeed.ySpeed;
    };
    const step = () => {
        trajectory.x += trajectory.xSpeed;
        trajectory.y += trajectory.ySpeed;
        if (trajectory.xSpeed > 0)
            trajectory.xSpeed--;
        else if (trajectory.xSpeed < 0)
            trajectory.xSpeed++;
        trajectory.ySpeed--;
    };
    const isInTargetArea = () => (targetArea.x1 <= trajectory.x &&
        trajectory.x <= targetArea.x2 &&
        targetArea.y1 <= trajectory.y &&
        trajectory.y <= targetArea.y2);
    const isBeyondTargetArea = () => trajectory.x > targetArea.x2 || trajectory.y < targetArea.y1;
    const velocities = [];
    for (let y = searchRange.y1; y <= searchRange.y2; y++) {
        // this could be optimized depending on the y value, e.g.
        // if y === searchRange.y1 there is a maximum of one step
        for (let x = searchRange.x1; x <= searchRange.x2; x++) {
            const speed = { xSpeed: x, ySpeed: y };
            reset(speed);
            let searchState = SearchState.InProgress;
            for (let i = 0, maxY = trajectory.y; searchState === SearchState.InProgress; i++) {
                step();
                if (trajectory.y > maxY)
                    maxY = trajectory.y;
                if (isInTargetArea()) {
                    searchState = SearchState.Hit;
                    velocities.push({ ...speed, ...{ maxHeight: maxY } });
                    break;
                }
                if (isBeyondTargetArea()) {
                    searchState = SearchState.Miss;
                    break;
                }
            }
        }
    }
    return velocities;
}
function getMaxY(input) {
    const velocities = getVelocities(input).sort((a, b) => b.ySpeed - a.ySpeed);
    return velocities[0].maxHeight;
}
exports.getMaxY = getMaxY;
function getUniqueValidVelocityCount(input) {
    return getVelocities(input).length;
}
exports.getUniqueValidVelocityCount = getUniqueValidVelocityCount;
//region internal
const partOne = (input) => getMaxY(input);
exports.partOne = partOne;
const partTwo = (input) => getUniqueValidVelocityCount(input);
exports.partTwo = partTwo;
// print solution to terminal if invoked directly
if (require.main === module) {
    const input = (0, fs_1.readFileSync)(__dirname + '/input.txt').toString();
    console.log((0, exports.partOne)(input));
    console.log((0, exports.partTwo)(input));
}
//endregion
